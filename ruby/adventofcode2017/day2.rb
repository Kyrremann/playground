...
3	8	6	5}
    ...
    assert_equal(18, checksum(values[0]))
    assert_equal(41887, checksum(values[1]))
    assert_equal(9, advance_checksum(values[2]))
    assert_equal(226, advance_checksum(values[1]))
  end
end
...
def advance_checksum(input)
  sum = 0
  input.split("\n").each do | line |
    min = 10000000
    max = 0
    temp_a = line.split("\t")
    temp_a.each_with_index do | n, i |
      n = n.to_i
      temp_a.each_with_index do | n2, i2 |
        next if i == i2
        n2 = n2.to_i
        if n % n2 == 0
          sum += n / n2
